...
def fitting_FU(filename_i, input_settings, export_data, input_fitting,
    ...
    # ----- Clean data: remove NaN/Inf; optional: restrict to positive measured forces -----
    ...
    # IMPORTANT: after invert(), x = distance, y = force
    fit_FU.add_data('Fully_unfolded', Force_data, Distance_data)

    # ----- Parameter initialization with safe bounds -----
    # Distance offset
    ...
    # Ensure strictly positive lower bounds
    ...
    fit_FU["ds_part/Lc"].fixed       = True  # boolean, not string

    # ssRNA (fixed here as in your code)
    ...
    # ----- CRITICAL: keep f_offset below min(Force_data) -----
    fmin = float(np.min(Force_data))
    eps = 1e-6

    # Start with the provided value but clamp it safely below fmin
    ...
    # If bounds are inconsistent, repair them
    if safe_upper <= safe_lower:
        # put the window just below fmin
        safe_upper = fmin - eps
    ...
    # (Optional) quick sanity: ensure min effective force > 0
    if (fmin - start_f_offset) <= 0:
        # nudge start below fmin a bit more
        fit_FU["ds_part/f_offset"].value -= max(0.1 * max(fmin, 1.0), 1e-3)
        fit_FU["RNA/f_offset"].value     = fit_FU["ds_part/f_offset"].value

    # ----- Fit -----
    # print("Force range:", np.min(Force_data), np.max(Force_data))
    # print("Distance range:", np.min(Distance_data), np.max(Distance_data))
    # print(f"Initial f_offset bounds/value: [{fit_FU['ds_part/f_offset'].lower_bound}, {fit_FU['ds_part/f_offset'].upper_bound}] -> {fit_FU['ds_part/f_offset'].value}")

    fit_FU.fit()
    fit_qual = fit_FU.log_likelihood()
    print(fit_FU.params)

    # ----- ds-only model for plotting (also inverted!) -----
    model_ds = (lk.ewlc_odijk_distance("ds_part").subtract_independent_offset()
                + lk.distance_offset("offset")).invert()
    fit_ds = lk.FdFit(model_ds)

    # IMPORTANT: inverted -> (distance, force)
...
def plot_fit(fit, start_force_ss, start_distance_ss, Force_Distance, Force_Distance_ds, save_folder, filename_i, start_time):
...
def fitting_FU_old(filename_i, input_settings, export_data, input_fitting, first_step_start,i_start,  i_end, Force_Distance, derivative_array, F_low, TOMATO_param):
    global model_ss
    global ss_fit_dict
