...
def openFile():
    ...
    directory4 = filedialog.askdirectory()    
    root.destroy()
...
font_size = 20
Plot_title=r"HOTAIR RNA +- 100 nM YBX1"  #+- 1 $\mu$M CspC 
data1_name = "RNA - unfolding"
data2_name = "RNA - refolding"
data3_name = "RNA+Protein - unfolding"
data4_name = "RNA+Protein - refolding"

line_thickness = 2
min_x_value, max_x_value = 900, 2699
min_y_value, max_y_value = -1 , 69

# Add a scale bar
scalebar_length = 200  # Length of the scale bar in x-axis units
...
def plot_data_from_directory(directory, color_hex, label, initial_alpha=1.0, alpha_step=0.10):
...
plot_data_from_directory(directory3, color_hex3, data3_name)
plot_data_from_directory(directory4, color_hex4, data4_name)
...
save_dir = r'W:\RNA folding\Nice_plotting'
os.makedirs(save_dir, exist_ok=True)  # This creates the directory if it doesn't exist

file_path_png = os.path.join(save_dir, f'{data_name}_FD_curves_2_pairs.png')
file_path_svg = os.path.join(save_dir, f'{data_name}_FD_curves_2_pairs.svg')
